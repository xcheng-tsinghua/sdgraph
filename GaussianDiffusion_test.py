...
class GaussianDiffusion(Module):
    def __init__(self, model, img_size, timesteps=1000):  # 1000
        super().__init__()
        print('gauss diffusion test model')
    ...
    def noise_pred_loss(self, x_0, t):
        ...
        :param x_0: 原始图片 [bs, n_stk, n_stk_pnt, 4]
        :param t: 当前时间步 [bs, ]
        ...
        nll = F.nll_loss(torch.log(noise_pred[..., 2:]).view(-1, 2), state.view(-1))
    ...
    def forward(self, img):
        """
        :param img: 原始草图 [bs, n_stk, n_stk_pnt, 4] xy and s one-hot
        :return: 噪音预测损失
        """
        bs = img.size(0)
        device = img.device
