...
class SDGraphSeg2(nn.Module):
    def __init__(self, channel_in, channel_out):
        super().__init__()
        print('diff 2025.2.19 添加avg的点坐标，只在笔划上添加')
        ...
        self.sd_down2 = SDGraphEncoder(sparse_l1, sparse_l2, dense_l1 + self.channel_in, dense_l2,
                                       self.n_stk, self.n_stk_pnt // 2,
                                       sp_near=2, dn_near=10,
                                       sample_type='down_sample',
                                       with_time=True, time_emb_dim=time_emb_dim)
        ...
        '''最终输出层'''
        final_in = dense_l0 + sparse_l0 + dense_l1 + sparse_l1 + self.channel_in
        self.final_linear = full_connected_conv1d(
            channels=[final_in, int((channel_out * final_in) ** 0.5), channel_out],
            final_proc=False
        )
    ...
    def forward(self, xy, time):
        ...
        '''下采样'''
        sparse_graph_up1, dense_graph_up1 = self.sd_down1(sparse_graph_up0, torch.cat([dense_graph_up0, xy], dim=1), time_emb)
        sparse_graph_up2, dense_graph_up2 = self.sd_down2(sparse_graph_up1, torch.cat([dense_graph_up1, xy_div2], dim=1), time_emb)
        ...
        '''上采样并融合UNet下采样阶段对应特征'''
        sparse_graph_down1, dense_graph_down1 = self.sd_up2(sparse_graph_down2, torch.cat([dense_graph_down2, xy_div4], dim=1), time_emb)  # -> [bs, sp_l2, n_stk], [bs, dn_l2, n_pnt]

        sparse_graph_down1 = torch.cat([sparse_graph_down1, sparse_graph_up1], dim=1)
        dense_graph_down1 = torch.cat([dense_graph_down1, dense_graph_up1], dim=1)

        sparse_graph_down0, dense_graph_down0 = self.sd_up1(sparse_graph_down1, torch.cat([dense_graph_down1, xy_div2], dim=1), time_emb)
