...
class SDGraphSeg2(nn.Module):
    ...
    def __init__(self, channel_in, channel_out):
        ...
        self.sd_down3 = SDGraphEncoder(sparse_l2, sparse_l3, dense_l2, dense_l3, self.n_stk, self.n_stk_pnt // 2,
                                       sp_near=2, dn_near=10, sample_type='down_sample')
        self.tm_sp_l3 = TimeMerge(sparse_l3, sparse_l3, time_emb_dim)
        self.tm_dn_l3 = TimeMerge(dense_l3, dense_l3, time_emb_dim)

        self.sd_down4 = SDGraphEncoder(sparse_l3, sparse_l4, dense_l3, dense_l4, self.n_stk, self.n_stk_pnt // 4,
                                       sp_near=2, dn_near=10, sample_type='none')
        self.tm_sp_l4 = TimeMerge(sparse_l4, sparse_l4, time_emb_dim)
        self.tm_dn_l4 = TimeMerge(dense_l4, dense_l4, time_emb_dim)

        '''全局特征生成层'''
        self.global_linear = full_connected(
            channels=[sparse_l4 + dense_l4, int(((sparse_l4 + dense_l4) * global_dim) ** 0.5), global_dim],
            final_proc=True)

        '''上采样层 × 2'''
        self.sd_up4 = SDGraphEncoder(global_dim + sparse_l4, sparse_l4,
                                     global_dim + dense_l4, dense_l4,
                                     n_stk=self.n_stk, n_stk_pnt=self.n_stk_pnt // 4,
                                     sp_near=2, dn_near=10, sample_type='none')
        self.tm_sp_dec_l4 = TimeMerge(sparse_l4, sparse_l4, time_emb_dim)
        self.tm_dn_dec_l4 = TimeMerge(dense_l4, dense_l4, time_emb_dim)

        self.sd_up3 = SDGraphEncoder(sparse_l4 + sparse_l3, sparse_l3,
                                     dense_l4 + dense_l3, dense_l3,
                                     n_stk=self.n_stk, n_stk_pnt=self.n_stk_pnt // 4,
                                     sp_near=2, dn_near=10, sample_type='up_sample')
        self.tm_sp_dec_l3 = TimeMerge(sparse_l3, sparse_l3, time_emb_dim)
        self.tm_dn_dec_l3 = TimeMerge(dense_l3, dense_l3, time_emb_dim)

        self.sd_up2 = SDGraphEncoder(sparse_l3 + sparse_l2, sparse_l2,
                                     dense_l3 + dense_l2, dense_l2,
                                     n_stk=self.n_stk, n_stk_pnt=self.n_stk_pnt // 2,
                                     sp_near=2, dn_near=10, sample_type='none')
        self.tm_sp_dec_l2 = TimeMerge(sparse_l2, sparse_l2, time_emb_dim)
        self.tm_dn_dec_l2 = TimeMerge(dense_l2, dense_l2, time_emb_dim)

        self.sd_up1 = SDGraphEncoder(sparse_l2 + sparse_l1, sparse_l1,
                                     dense_l2 + dense_l1, dense_l1,
                                     n_stk=self.n_stk, n_stk_pnt=self.n_stk_pnt // 2,
                                     sp_near=2, dn_near=10, sample_type='up_sample')
    ...
    def forward(self, xy, time):
        ...
        sparse_graph_up3, dense_graph_up3 = self.sd_down3(sparse_graph_up2, dense_graph_up2)
        sparse_graph_up3 = self.tm_sp_l3(sparse_graph_up3, time_emb)
        dense_graph_up3 = self.tm_dn_l3(dense_graph_up3, time_emb)

        sparse_graph_up4, dense_graph_up4 = self.sd_down4(sparse_graph_up3, dense_graph_up3)
        sparse_graph_up4 = self.tm_sp_l4(sparse_graph_up4, time_emb)
        dense_graph_up4 = self.tm_dn_l4(dense_graph_up4, time_emb)

        '''获取全局特征'''
        sparse_global = torch.max(sparse_graph_up4, dim=2)[0]
        dense_global = torch.max(dense_graph_up4, dim=2)[0]
        ...
        '''将 sd_graph 融合全局特征'''
        sparse_fit = fea_global.unsqueeze(2).repeat(1, 1, self.n_stk)
        sparse_graph_down4 = torch.cat([sparse_graph_up4, sparse_fit], dim=1)

        dense_fit = fea_global.unsqueeze(2).repeat(1, 1, dense_graph_up4.size(2))
        dense_graph_down4 = torch.cat([dense_graph_up4, dense_fit], dim=1)

        '''上采样并融合UNet下采样阶段对应特征'''
        sparse_graph_down3, dense_graph_down3 = self.sd_up4(sparse_graph_down4, dense_graph_down4)
        sparse_graph_down3 = self.tm_sp_dec_l4(sparse_graph_down3, time_emb)  # -> [bs, sp_l2, n_stk]
        dense_graph_down3 = self.tm_dn_dec_l4(dense_graph_down3, time_emb)  # -> [bs, dn_l2, n_pnt]

        sparse_graph_down3 = torch.cat([sparse_graph_down3, sparse_graph_up3], dim=1)
        dense_graph_down3 = torch.cat([dense_graph_down3, dense_graph_up3], dim=1)

        sparse_graph_down2, dense_graph_down2 = self.sd_up3(sparse_graph_down3, dense_graph_down3)
        sparse_graph_down2 = self.tm_sp_dec_l3(sparse_graph_down2, time_emb)
        dense_graph_down2 = self.tm_dn_dec_l3(dense_graph_down2, time_emb)

        sparse_graph_down2 = torch.cat([sparse_graph_down2, sparse_graph_up2], dim=1)
        dense_graph_down2 = torch.cat([dense_graph_down2, dense_graph_up2], dim=1)
